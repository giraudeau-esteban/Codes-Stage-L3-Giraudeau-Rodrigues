...
def simili_anneau(R1, R2, N1, N2, a=1, b=1, theta=0):
    ...
    P1 = R1*np.exp(np.complex(0, 1)*(2*np.pi/N1*np.arange(0, N1+1)+2))
    P2 = R2*np.exp(-np.complex(0, 1)*(2*np.pi/N1*np.arange(0, N2+1)+2))

    R = np.zeros((N1+N2, 2, 2))

    R[0:N1, 0, 0] = np.real(P1[0:-1])/a
    R[0:N1, 0, 1] = np.imag(P1[0:-1])/b
    R[0:N1, 1, 0] = np.real(P1[1:N1+1])/a
    R[0:N1, 1, 1] = np.imag(P1[1:N1+1])/b

    R[N1:N1+N2, 0, 0] = np.real(P2[0:-1])/a
    R[N1:N1+N2, 0, 1] = np.imag(P2[0:-1])/b
    R[N1:N1+N2, 1, 0] = np.real(P2[1:N1+1])/a
    R[N1:N1+N2, 1, 1] = np.imag(P2[1:N1+1])/b
...
Ann = simili_anneau(2.5, 1.8, 25, 25, 1, 2, 1)

Rec = rectangle(np.array([0, 0]), np.pi/6, 1, 2)

A,W,B, S, H, V=AWB_polygone(Ann, N, M, L, l)

# for seg in Rec:
#     plt.plot(seg[:, 0], seg[:, 1])

plt.imshow(S, extent=[0,L,0,l],aspect='auto', cmap='bwr')
#
plt.axis('scaled')
