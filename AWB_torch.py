...
def pentagone(centre, theta, A, B, h):
    ...
    rot = torch.tensor([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
    p1,p2,p3,p4, p5=torch.zeros([2], dtype=torch.double), torch.zeros([2], dtype=torch.double),torch.zeros([2], dtype=torch.double),torch.zeros([2], dtype=torch.double),torch.zeros([2], dtype=torch.double)
